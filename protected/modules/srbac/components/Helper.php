...
class Helper {
  ...
  //对象转为数组
  public static function objToArr2($models){
    $res = array();
    foreach ($models as $key => $value) {
  ...
  //对象转为数组
  public static function excelTotime($time){
    $n = intval(($time - 25569) * 3600 * 24); //转换成1970年以来的秒数
    return gmdate('Y-m-d H:i:s', $n);//格式化时间;
  } 
  //客户代理
  public static function getAgentLike($content){
    $res = array(
      1 => '省代',
      2 => '市代',
      3 => '县代',
    ...
    foreach ($res as  $key=>$value) {
      $tmparr = explode($value,$content);
      $tmparr2 = explode($content,$value);
      if(count($tmparr)>1||count($tmparr2)>1){
        $agentid=$key;
        break;
      }
    }
    if($agentid==5) $agentid=3;
    if($agentid==6) $agentid=2;
    $agentid?$agentid:2;
    return $agentid;
  }
  // 获取管理员
  public static function getUserName($content){
    $userList=Admins::model()->findAll(array('select'=>'userid,realname','condition'=>'validate = 1','order'=>'userid desc','limit'=>8));
    $res=0;
    foreach ($userList as  $value) {
      $in=strstr($content,$value['realname']);
      if($in){
        $res=$value['userid'];
        break;
      }

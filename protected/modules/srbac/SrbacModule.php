...
//Constants
  ...
  public function getAssetUrl(){
    if($this->_assetsUrl === null){
      $this->_assetsUrl = Yii::app()->getAssetManager()->publish(Yii::getPathOfAlias('application.modules.srbac.assets',true, -1, true));
    }
  ...
  public function init() {
    ...
    //Set layout to main
    if($this->layout =="") {
      $this->layout = "application.modules.srbac.layouts.admin";
    }
    //Publish css
    ...
    $this->setComponents(
      ...
      'user'=>array(
        'class'=>'CWebUser',
        'stateKeyPrefix'=>'vipcrm_srbac',
        'loginUrl'=>Yii::app()->createUrl('/default/login'),
        'guestName'=>'模块里面的默认用户'
      ),
      )
    );
  ...
  public function beforeControllerAction($controller, $action) {
    if(parent::beforeControllerAction($controller, $action)) {
      ...
      if(!Yii::app()->controller->module->getComponent('user')->isGuest){
        //判断是否有权限
        if(Yii::app()->controller->module->getComponent('user')->checkAccess($controller->id)){
          return true;
        }else{
        ...
        //跳转到登陆页面
        Yii::app()->request->redirect(Yii::app()->createUrl('/srbac/default/login'));
      }
    }else{
      return false;
    }

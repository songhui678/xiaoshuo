...
class Helper extends CController
{
    ...
    public static function truncate_utf8_string($string, $length, $etc = '...')
    {
        ...
        for ($i = 0; (($i < $strlen) && ($length > 0)); $i++) {
            if ($number = strpos(str_pad(decbin(ord(mb_substr($string, $i, 1))), 8, '0', STR_PAD_LEFT), '0')) {
                if ($length < 1.0) {
                    break;
                }
                $result .= mb_substr($string, $i, $number);
                $length -= 1.0;
                $i += $number - 1;
            } else {
    ...
    // 自动生成.
    public static function GrabImage($url, $filepath = "/upload/video_photo/", $filename = "")
    {
        if ($url == "") {
            return false;
        }
        ...
        ob_end_clean();
        $size     = strlen($img);
        $filepath = $_SERVER['DOCUMENT_ROOT'] . $filepath . date('Y-m-d') . "/";
        if (!is_dir($filepath)) {

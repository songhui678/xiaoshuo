...
class YoukuUtils
{
    ...
    public static function getYk($url)
    {
        // 这段正则是来获取优酷的id，出处在 /wp-content/languages/zh_CN.php，同样56网、土豆都可以找到
        $link = '';
        if (!empty($url)) {
            $link = "http://play.youku.com/play/get.json?vid=" . $url . "==&ct=10";
        } else {
            return false;
        }
        ...
        if ($cexecute) {
            ...
            $data['seconds']        = $json['seconds']; //视频播放所需时间
            $data['published_time'] = $json['published_time']; //视频发布时间
            $data['url']            = $url;
            return $data;
        } else {
            return false;
    ...
    public static function _getYouku($vid)
    {
        ...
        if ($retval) {
            ...
            $ku['img']     = $rs['data'][0]['logo'];
            $ku['title']   = $rs['data'][0]['title'];
            $ku['seconds'] = $rs['data'][0]['seconds'];
            return $ku;
        } else {
            return false;
        }
